        ...
        function setPgUI() {
            ...
            var tlBar = new Ext.ux.AimToolbar({
                items: [{ text: '查询:' }, new Ext.app.AimSearchField({ schbutton: true, aimhandler: function(val) {
                    ...
                    $.ajaxExec("getData", { filedVal: val }, function(rtn) {
                        var Ents = rtn.data.DataList;
                        var EntRecord = store.recordType;
                        store.removeAll();
                        for (var i = 0; i < Ents.length; i++) {

                            var rec = new EntRecord({
                                RoleID: Ents[i].RoleID,
                                Name: Ents[i].Name,
                                Code: Ents[i].Code,
                                Type: Ents[i].Type,
                                SortIndex: Ents[i].SortIndex,
                                Description: Ents[i].Description
                            });
            ...
            { id: 'Name', header: '角色名', width: 200, sortable: true, dataIndex: 'Name', renderer: RowRender },
            { id: 'Code', header: '编号', width: 80, sortable: true, dataIndex: 'Code' },
