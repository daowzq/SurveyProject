<%@ Page Title="审批人员" Language="C#" MasterPageFile="~/Masters/Ext/formpage.Master"
    AutoEventWireup="true" CodeBehind="WorkFlowChoices.aspx.cs" Inherits="Aim.Examining.Web.SurveyManage.WorkFlowChoices" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadHolder" runat="server">
    ...
    <script type="text/javascript">
        ...
        function setPgUI() {
            ...
            if (arr[0] == "1") {
                ...
                if (arr.length > 3) {
                    ...
                    //                    }
                    var temp = arr[3] || "";
                    $("#wfTxt").after(temp ? "<font color=red>,审批对象须为:&nbsp;&nbsp;</font>" + temp : temp);
                }
        ...
        function SuccessSubmit() {
            if ($("#MustApprove").val() && !$("#UserName1").val()) {
                AimDlg.show("该问卷必须审批,请指定审批人!");
                return;
            }

            //            if (ApproveRole.ApproveUses) {
            //var arr = ApproveRole.ApproveUses.split(",");
            var validate = false;
            var submitUsr = $("#UserId1").val() + ""; //提交人员

            $.ajaxExec("CheckUsr", { SurveyId: SurveyId, UserIds: submitUsr }, function(rtn) {
                if (rtn.data.State == "1") {
                    ...
                    AimDlg.show("选择人员的职位与签核职位不配置!");
                    return;
                }
            });

            //                for (var i = 0; i < arr.length; i++) {
            //                    submitUsr.indexOf(arr[i]) > -1 && (validate = true);
            //                }
            //                if (!validate) {
            //                    AimDlg.show("请选择职位为: " + "\r\n<b>" + ApproveRole.ApproveRoleName + "</b> 的人员");
            //                    return;
            //                }
            //}
